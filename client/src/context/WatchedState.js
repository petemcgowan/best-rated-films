...
export const WatchedProvider = ({ children }) => {
  ...
  async function getWatched() {
    try {
      ...
      const res = await axios.post('/api/watched', {"email": getEmail()}, config);
      console.log ("WatchedContext/State, getWatched():" + JSON.stringify(res.data.data));
  ...
  async function addWatched(title, email) {
    ...
    try {
      ...
      const res = await axios.post('/api/watched/addWatched', { title, email }, config);
